...
ptm = {
	...
	logs:0,                     // What rolls downstairs, alone or in pairs?
    ...
    onState:{
		...
		launch:function(){
            ...
            if(this.y > this.tempY){
		        ...
		        this.dy = ((15 * (this.tempY - this.y)) / this.tempY + 1)*-1;
		...
		flying:function(){
			...
			this.dx = Math.cos(this.radian-Math.PI) * 5;
			this.dy = Math.sin(this.radian-Math.PI) * 5;
...
draw = function(ctx){
	...
	var startX = Math.abs(viewPort.x) % 50;
	...
	// draw buildings
	/*
	var i=0,len=buildings.length;
	
	
	ctx.fillStyle='#ffff00';
	while(i<len){
	
	    pos = viewPort.getVPRelative(buildings[i].x, map.horizon - buildings[i].h);	    
	    ctx .fillRect(pos.x,pos.y, buildings[i].w, buildings[i].h);
	...
	*/
	...
	// draw target
	pos = viewPort.getVPRelative(ptm.target.x, map.horizon - ptm.target.size);
	ctx.fillStyle='#ff00ff';
	ctx.fillRect(pos.x,pos.y, ptm.target.size, ptm.target.size );
	...
	// draw pointer to target
		
	var angles = _.getAngles(ptm, ptm.target);
    ctx.save();
	ctx.translate(pos.x+ptm.w/2,pos.y+ptm.h/2);	
	...
	ctx.fillStyle='#ffff00';
	ctx.fillText('distance: '+ _.distance(ptm.x,ptm.y,ptm.target.x,ptm.target.y).toFixed(2),10,10);
...
start = function(){
	...
	sheet.addEventListener('load', function(e){
		...
		// start thread when sprites are loaded
		thread();
...
newGame = function(){
    ...
    ptm.target.x = Math.floor(Math.random()*-2500 - 500);
	...
	/*
	var i = 0, len = 1,
	totalSections = Math.floor( (map.width-500) / 100),
	sections = Array.apply(null, {length: totalSections}).map(Number.call, Number),
	...
	while(i < len){
	    ...
	    sectionSize = Math.floor(Math.random()*3)+1;
	    
		sIndex = Math.floor(Math.random() * sections.length) - sectionSize;
		
		placeAt = sections.splice(sIndex, sectionSize);
		
		console.log(placeAt);		
		console.log((100 * placeAt[0] + 600) * -1);
		console.log('');
		
		buildings.push( new Building( (100 * placeAt[0] + 600) * -1, sectionSize * 100, 128 ) );
		
	    //buildings.push( new Building( Math.random() * -2500 - 500, sectionSize * 100, 100) );
...
*/
	...
	// make one of the buildings the target
    

    ptm.flightState='ready';
