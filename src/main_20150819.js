...
ptm = {
	...
	winLostCheck : function(){
		...
		if(_.boundingBox(this.x, this.y,10,10,this.target.x, this.target.y,this.target.width, this.target.height)){
    ...
    onState:{
		...
		bardrag:function(){
			...
			this.radian =  -(Math.PI * toastBar.setting);
        
            this.x = Math.cos(this.radian) * 120 + this.tempX-120;
            this.y = Math.sin(this.radian) * 120 + this.tempY;
...
dial = {
    ...
    setting : 0.5,  // a value from 0 to 1 that repersents a setiing between a min and max
...
draw = function(ctx){
    ...
    document.getElementById('out').innerHTML='<br>'+
	    'flight state: ' + ptm.flightState + ', flightPower: ' + ptm.flightPower;
